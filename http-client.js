...
define([
	...
	'./util/querystring',
	'./util/uri',
	'./engines/node/http-client' // FIXME
],
function(
	defer,
	platform,
	querystring,
	uri,
	_nodeHttpClient // FIXME
) {
	/**
	* HTTP Client using the JSGI standard objects
	*/

	var print = platform.print;
	var request;
	if (typeof XMLHttpRequest === 'undefined'){
		request = _nodeHttpClient.request;
	}
	else {
	...
	request.Redirect = function(nextApp, maxRedirects){
		...
		return function(request){
			var remaining = maxRedirects,
				deferred = defer();
			function next(){
				defer.when(nextApp(request), function(response) {
					if(remaining--){
	...
	request.CookieJar = function(nextApp) {
		...
		return function(req) {
			...
			return defer.when(nextApp(req), function(response) {
				var cookies;
