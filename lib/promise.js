...
function Deferred(canceller){
	...
	//freeze(promise); // is this necessary?
};
 ...
 * 
 * In addition to functions with no arguments you can pass an array modeled on Function::call
 * This array should have the function, target, and any subsequent parameters
 *   whenCall([object.save, object, directives], successHandler, errorHandler);
 */
exports.whenCall = function(target, resolvedCallback, rejectCallback, progressCallback) {
    // restructure array arguments and pass to whenApply
    if (Array.isArray(target)) {
        target[2] = target.slice(2);
        target.length = 3;
    }
    return exports.whenApply(target, resolvedCallback, rejectCallback, progressCallback);
};

/**
 * The whenApply function is a bit more generic than whenCall, so this is where the action happens.
 * This can be called with just a function or with a set of arguments modeled on Function:apply
 *   whenApply([object.get, object, [id, directives]], successHandler, errorHandler);
 */
exports.whenApply = function(target, resolvedCallback, rejectCallback, progressCallback) {
    // handle naked functions
    if (typeof target === 'function')
        target = [target, null, []];
    // fail early when no function is provided
    if (!target || typeof target[0] !== 'function')
        throw new Error('Invalid argument to whenApply: ' + target);
	try {
		return exports.when(target[0].apply(target[1], target[2]), resolvedCallback, rejectCallback, progressCallback);
	}
	catch (e){
 ...
 * If there are no promises in the provided array (or arguments) the array is returned
...
exports.all = function(array) {
	...
	if (array.every(function(i) { return !isPromise(i); })) return array;
	var deferred = new Deferred();
	var fulfilled = 0;
...
exports.allKeys = function(hash){
	...
	if (array.every(function(i) { return !isPromise(hash[i]) })) return hash;
	var deferred = new Deferred();
	var fulfilled = 0;
	var length = array.length;
	var results = {};
	array.forEach(function(key){
		exports.when(hash[key],
			function(value){
				results[key] = value;
				if ((fulfilled++) === length) deferred.resolve(results);
			},
			deferred.reject);
	});
	return deferred.promise;
...
exports.first = function(array){
	...
	array.forEach(function(promise, index){
		...
		function(error){
			if (!fulfilled) {
				fulfilled = true;
				deferred.resolve(error); // FIXME deferred.reject?
			}	
		});
	});
	return deferred.promise;
 ...
 * executes them sequentially. Each function is called with the return value of the last function
