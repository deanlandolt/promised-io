...
define(["../promise", "../console", "require"],
function(promise, console, require){
...
request.get = function() {
    return promise.whenApply([request, this, arguments], function(response) {
        return when(response.body, function(body) {
            if (response.status < 400) {
                // FIXME support binary
                var str = "";
                return when(body.forEach(function(i) { str += i; }), function() {
                    return str;
                });
            }
		...
		function next(){
			when(nextApp(request), function(response) {
				if(remaining--){
					...
					if(isRedirect(response)) console.warn("Maximum redirects reached")
					deferred.resolve(response);
				}
			}, deferred.reject);
		}
...
request.CookieJar = function(nextApp) {
  ...
  return function(req) {
    ...
    var parseUri    = require("../util/uri").parseUri;
