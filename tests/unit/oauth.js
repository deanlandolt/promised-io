define([
	...
	registerSuite({
		name: 'promised-io/oauth',

		'generate signature base string': function () {
			...
			assert.strictEqual(result, "GET&http%3A%2F%2Fphotos.example.net%2Fphotos&file%3Dvacation.jpg%26oauth_consumer_key%3Ddpf43f3p2l4k3l03%26oauth_nonce%3Dkllo9940pd9333jh%26oauth_signature_method%3DHMAC-SHA1%26oauth_timestamp%3D1191242096%26oauth_token%3Dnnch734d00sl2jdk%26oauth_version%3D1.0%26size%3Doriginal");
		},
		...
		'normalize url': function () {
			...
			assert.strictEqual(client._normalizeUrl({ protocol: "https:", hostname: "somehost.com", port: "443", pathname: "/foo/bar" }), "https://somehost.com/foo/bar");
			assert.strictEqual(client._normalizeUrl({ protocol: "http:", hostname: "somehost.com", port: "80", pathname: "/foo/bar" }), "http://somehost.com/foo/bar");

			// should leave non-default ports from URLs for use in signature generation
			assert.strictEqual(client._normalizeUrl({ protocol: "https:", hostname: "somehost.com", port: "446", pathname: "/foo/bar" }), "https://somehost.com:446/foo/bar");
			assert.strictEqual(client._normalizeUrl({ protocol: "http:", hostname: "somehost.com", port: "81", pathname: "/foo/bar" }), "http://somehost.com:81/foo/bar");
		},

		'normalize request params': function () {
			var client = new oauth.Client;

			// ordered by name
			assert.strictEqual(client._normalizeParams(["z", "a", "a", "b", "1", "c"]), "1=c&a=b&z=a");

			// if two parameter names are the same then order by the value
			assert.strictEqual(client._normalizeParams(["z", "b", "z", "a", "1", "c"]), "1=c&z=a&z=b");
			...
			assert.strictEqual(client._normalizeParams(requestParams), "a2=r%20b&a3=2%20q&a3=a&b5=%3D%253D&c%40=&c2=&oauth_consumer_key=9djdj82h48djs9d2&oauth_nonce=7d8f3e4a&oauth_signature_method=HMAC-SHA1&oauth_timestamp=137131201&oauth_token=kkk9d7dh3k39sjv7");
		},
		...
		'sign url': {
			'without token': function () {
				...
				assert.strictEqual(signature, "7ytO8vPSLut2GzHjU9pn1SV9xjc=");
			},
			...
			'with token': function () {
				...
				assert.strictEqual(oauthParams.oauth_token, "token");
				assert.strictEqual(signature, "9LwCuCWw5sURtpMroIolU3YwsdI=");
			},
			...
			'with token and secret': function () {
				...
				var baseString = bound._createSignatureBase("GET", "http://somehost.com:3323/foo/poop", params);
				var signature = bound._createSignature(baseString);
				assert.strictEqual(signature, "zeOR0Wsm6EG6XSg0Vw/sbpoSib8=");
			}
		},
		...
		'build auth header': function () {
			...
			assert.strictEqual(request.headers.authorization, 'OAuth oauth_consumer_key="consumerkey",oauth_signature_method="HMAC-SHA1",oauth_timestamp="1272399856",oauth_nonce="ybHPeOEkAUJ3k2wJT9Xb43MjtSgTvKqp",oauth_version="1.0",oauth_signature="7ytO8vPSLut2GzHjU9pn1SV9xjc%3D"'); 
		}
	});
});
